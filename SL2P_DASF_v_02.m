...
for ivar=1:length(bio_vars),
    ...
    output_too_high(find(bio_sim(:,3)>bounding_box.Pmin+bounding_box.Tolerance),:)=1;
    ...
    flags=(2^0)*input_out_of_range+(2^1)*output_thresholded_to_min_outpout+(2^2)*output_thresholded_to_max_outpout+...
        (2^3)*output_too_low+(2^4)*output_too_high+(2^5)*no_bare_soil_or_vegetated_area;

    eval(['NNT_OUT.',lower(bio),'=reshape(bio_sim(:,end),r,c);']);
